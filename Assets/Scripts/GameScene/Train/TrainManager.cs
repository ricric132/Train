...
public class TrainManager : MonoBehaviour
{
    ...
    public List<Seat> GetAvailableSeats()
    {
        ...
        {
            if (activeSeats[i].occupiedGO == null)
            {
                emptySeats.Add(activeSeats[i]);
            }
        }
    ...
    public TrainCar GetTrainCar(Seat _seat)
    {
        return trainCars[(int)(seats.IndexOf(_seat)/3)];
    }
    ...
    {
        List<Seat> res = new List<Seat>();  
        int carNum = trainCars.IndexOf(car);

        for (int i = carNum*3; i < (carNum+1)*3; i++) 
        {
            res.Add(seats[i]);
        }
